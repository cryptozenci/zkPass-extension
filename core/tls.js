...
export class TLS {
  ...
  async parseServerKeyExchange(s) {
    ...
    assert(eq(s.slice(p, p += 2), [0x04, 0x01]));
  ...
  async receiveAndParseServerHello() {
    try {
      ...
      throw ('Failed to receive a response from a webserver.');
    }
    ...
    if (eq(s.slice(0, 2), [0x15, 0x03])) {
      throw ('Unfortunately zkPass is not yet able to notarize this website.');
    }
