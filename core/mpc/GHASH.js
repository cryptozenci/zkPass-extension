...
export class GHASH {
  ...
  // prepareOTRequest prepares a request for zkPass's masked XTables
  ...
  // Add zkPass's masked XTables to our tag share
  ...
  // processFinResponse processes zkPass's OT response and multiplies
  ...
  async blockAggregationBuildRequest(ghashInputs){
    ...
    // request zkPass's masked X-table for each zkPass's small power a.
    // We send bits of our aggregated values
    // and also
    // request zkPass's X-table for each zkPass's aggregated value. We send bits
    // of our small power
  ...
  // add zkPass's XTable to our share of the tag
...
// the bits of y. The zkPass supplies the XTable.
